#!/usr/bin/env python3
"""
Evaluate next-token distribution differences on virus validation sequences.

Sequences are loaded from a CSV file whose first column is the nucleotide
sequence (remaining columns are ignored). Random segments of fixed length are
sampled and compared between the Focus Token adapter and the base model.
"""
...
DEFAULT_CSV = ROOT / "data" / "Virus" / "virus_val.csv"


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare next-token distributions using segments sampled from virus validation sequences."
    )
...
def load_sequences(csv_path: Path) -> List[Tuple[str, str]]:
    ...
    with csv_path.open("r", encoding="utf-8") as fh:
        ...
        for idx, row in enumerate(reader):
            if not row:
                continue
            if not header_processed and row[0].lower() == "sequence":
            ...
            seq_id = f"virus_{idx}"
            if label:
                seq_id = f"{seq_id}_{label}"
            sequences.append((seq_id, sequence))

    if not sequences:
        raise ValueError(f"No sequences parsed from CSV: {csv_path}")
    return sequences
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    default_dir = args.run_dir / "outputs" / "Virus"
    output_dir = args.output_dir or default_dir
    output_dir.mkdir(parents=True, exist_ok=True)
    header_fields = ["sequence_id", "start", "end", "L1", "L2", "Hellinger", "Jensen-Shannon", "KL(base||focus)"]
